...
function startImageLoop(rate) {
    ...
    imageInterval = setInterval(() => {
        ...
        // Safety: Only update if elements exist (Mini mode has no cams)
...
async function toggleCamera(id, isEnabled) {
    // Safety: Mini mode has no toggles
    if (!cam1Card) return;
...
function setButtonState(mode) {
    if (!forceStartBtn) return; // Safety
...
function applyLayout(count) {
    if (!cameraGrid) return; // Safety for Mini Mode
...
async function loadSettings() {
    try {
        ...
        if (cam1Toggle) { // Only if full dashboard
